...
namespace PackIT.UnitTest.Application
{
    ...
    {
        Task Act(CreatePackingListWithItems command) => _commandHandler.HandlerAsync(command);

        [Fact]
        public async Task HandleAsync_Throws_PackingListAlreadyExistsException_When_List_With_Same_Name_Already_Exists()
        {
            var command = new CreatePackingListWithItems(Guid.NewGuid(), "MyList", 10, Gender.Female, new LocalizationWriteModel("Xian", "China"));
        ...
        public async Task HandleAsync_Calls_Repository_On_Success()
        {
            ...
            _factory.Create(command.Id, command.Name, command.Days, command.Gender,
